import type { CardBand, DataCompleteness, HoldingCard } from '../../dss/holdingCard';
...
export function HoldingCardView({ card }: { card: HoldingCard }) {
  const { analysis, position } = card;

  return (
    <article className="card" aria-label={`${card.stockId} ${card.stockName}`}>
      <header className="card__head">
        <div className="card__identity">
          <span className="card__id num">{card.stockId}</span>
          <span className="card__name">{card.stockName}</span>
          <span className="card__class">{ASSET_LABEL[card.assetClass]}</span>
          <span className="card__class">{card.tradeType}</span>
        </div>
        <span
          className={`card__completeness card__completeness--${card.completeness}`}
          title={`市場資料 ${card.priceDate ?? '未取得'}．庫存快照 ${card.snapshotDate}`}
        >
          {COMPLETENESS_LABEL[card.completeness]}
        </span>
      </header>
